...
# Pruning configuration - loaded from config
PRUNING_CONFIG = get_pruning_config()

# Data configuration - loaded from config
...
def experiment_1a_train_all_models(
    models_to_train=None, save_plots=True, verbose=True, global_test_indices=None
):
    """
    ...
    for idx, model_name in enumerate(models_to_train, 1):
        ...
        try:
            ...
            if verbose:
                info(
                    f"Preparing to train {variants_per_model} variants for {model_name}..."
                )
            ...
            for variant_idx in range(1, variants_per_model + 1):
                ...
                if backbone_dataset is not None:
                    ...
                    if GLOBAL_TEST_INDICES is not None:
                        ...
                        remaining_indices = [
                            i for i in all_indices if i not in test_indices
                        ]

                        # Shuffle remaining indices per-variant to alter train/val split
                        gen = _torch.Generator().manual_seed(42 + variant_idx)
                        perm_remaining = _torch.randperm(
                            len(remaining_indices), generator=gen
                        ).tolist()
                        shuffled_remaining = [
                            remaining_indices[i] for i in perm_remaining
                        ]
                ...
                if verbose:
                    ...
                    info(
                        f"Max epochs: {TRAINING_CONFIG['max_epochs']}, Patience: {TRAINING_CONFIG['patience']}"
                    )
                ...
                test_loss = test_model(
                    best_model, test_loader, criterion, TRAINING_CONFIG["device"]
                )
                test_rmse = np.sqrt(test_loss)  # Compute RMSE from MSE

                # Save model weights for this variant
                weights_path = (
                    WEIGHTS_DIR / f"{model_name}_exp1a_variant{variant_idx}_best.pth"
                )
                ...
                history_path = (
                    ...
                    / f"{model_name}_exp1a_variant{variant_idx}_history.npy"
                )
                ...
                if save_plots:
                    try:
                        ...
                        plot_path = (
                            TRAINING_PLOTS_DIR
                            / f"{model_name}_exp1a_training_curve_variant{variant_idx}.png"
                        )
                ...
                info(
                    f"  - Best Val Loss: {results[model_name][f'variant{variant_idx}']['best_val_loss']:.4f}"
                )
                info(
                    f"  - Epochs trained: {results[model_name][f'variant{variant_idx}']['epochs_trained']}"
                )
            ...
            try:
                if model_name in results and isinstance(results[model_name], dict):
                    ...
                    if variant_items:
                        ...
                        best_variant, best_data = min(
                            variant_items, key=lambda iv: iv[1]["final_test_mse"]
                        )
                        # Averages across variants
                        avg_mse = float(
                            np.mean([v["final_test_mse"] for _, v in variant_items])
                        )
                        avg_rmse = float(
                            np.mean([v["final_test_rmse"] for _, v in variant_items])
                        )

                        # Populate model-level summary fields
                        results[model_name]["best_variant"] = best_variant
                        results[model_name]["final_test_mse"] = best_data[
                            "final_test_mse"
                        ]
                        results[model_name]["final_test_rmse"] = best_data[
                            "final_test_rmse"
                        ]
                        ...
                        results[model_name]["weights_path"] = best_data.get(
                            "weights_path"
                        )
                        # Add some aggregated training info
                        try:
                            results[model_name]["best_val_loss"] = float(
                                min(
                                    [
                                        v.get("best_val_loss", np.inf)
                                        for _, v in variant_items
                                    ]
                                )
                            )
                        except Exception:
                            pass
                        try:
                            results[model_name]["epochs_trained"] = int(
                                np.round(
                                    np.mean(
                                        [
                                            v.get("epochs_trained", 0)
                                            for _, v in variant_items
                                        ]
                                    )
                                )
    ...
    for model_name, model_results in results.items():
        ...
        for variant_key, variant_data in model_results.items():
            if isinstance(variant_data, dict) and "final_test_mse" in variant_data:
                all_variants.append(
                    (
                        f"{model_name}_{variant_key}",
                        variant_data["final_test_mse"],
                        variant_data["final_test_rmse"],
                    )
                )
...
def experiment_1b_prune_all_models(
    ...
    if pruning_method == "both":
        ...
        raise ValueError(
            f"Unknown pruning method: {pruning_method}. Use 'greedy', 'negative_impact', or 'both'."
        )
    ...
    for idx, (model_name, weight_file_list) in enumerate(model_items, 1):
        ...
        try:
            ...
            for weights_path in weight_file_list:
                ...
                importance_path = (
                    RANKINGS_DIR
                    / f"{model_name}_exp1b_{variant_tag}_importance_scores.npy"
                )
                ...
                plot_path = (
                    RANKING_PLOTS_DIR
                    / f"{model_name}_exp1b_{variant_tag}_importance_scores.png"
                )
                try:
                ...
                for method in methods_to_run:
                    if method == "greedy":
                        ...
                        pruned_weights_path = (
                            WEIGHTS_DIR
                            / f"{model_name}_exp1b_{variant_tag}_greedy_pruned.pth"
                        )
                        ...
                        info(
                            f"Performing negative impact pruning (threshold={threshold})..."
                        )
                        pruned_weights_path = (
                            WEIGHTS_DIR
                            / f"{model_name}_exp1b_{variant_tag}_negative_pruned.pth"
                        )
                    ...
                    # Store results
                    results.setdefault(model_name, {}).setdefault(variant_tag, {})[
                        method
                    ] = method_results

                    # Print method-specific results
                    info(
                        f"✓ {model_name.upper()} {variant_tag} {method} pruning complete!"
                    )
                    info(
                        f"  - Baseline MSE: {pruning_results['baseline_mse']:.4f}, RMSE: {pruning_results['baseline_rmse']:.4f}"
                    )
                    info(
                        f"  - Final MSE: {pruning_results['final_mse']:.4f}, RMSE: {pruning_results['final_rmse']:.4f}"
                    )
                    info(
                        f"  - Improvement MSE: {pruning_results['improvement_mse']:.4f}, RMSE: {pruning_results['improvement_rmse']:.4f}"
                    )
                    info(
                        f"  - Features removed: {len(pruning_results['removed_features'])}"
                    )
                    info(
                        f"  - Reduction: {pruning_results['reduction_percentage']:.1f}%"
                    )

            # After processing all variants for this model, aggregate results across variants for each method
            for method in methods_to_run:
                variant_results = [
                    results[model_name][vt][method]
                    for vt in results[model_name]
                    if isinstance(results[model_name][vt], dict)
                    and method in results[model_name][vt]
                ]
                if variant_results:
                    # Compute averages across variants
                    avg_baseline_mse = float(
                        np.mean([r["baseline_mse"] for r in variant_results])
                    )
                    avg_baseline_rmse = float(
                        np.mean([r["baseline_rmse"] for r in variant_results])
                    )
                    avg_final_mse = float(
                        np.mean([r["final_mse"] for r in variant_results])
                    )
                    avg_final_rmse = float(
                        np.mean([r["final_rmse"] for r in variant_results])
                    )
                    avg_improvement_mse = float(
                        np.mean([r["improvement_mse"] for r in variant_results])
                    )
                    avg_improvement_rmse = float(
                        np.mean([r["improvement_rmse"] for r in variant_results])
                    )
                    avg_num_removed = float(
                        np.mean([r["num_removed"] for r in variant_results])
                    )
                    avg_reduction_percentage = float(
                        np.mean([r["reduction_percentage"] for r in variant_results])
                    )
    ...
    info(
        f"Successfully pruned: {len(successful_models)}/{len(weights_files_by_model)} models"
    )
    ...
    if successful_models:
        if len(methods_to_run) > 1:
            ...
            for method in methods_to_run:
                ...
                info(
                    f"{'Model':<20} {'Base MSE':<10} {'Base RMSE':<11} {'Final MSE':<10} {'Final RMSE':<11} {'Δ MSE':<10} {'Removed':<10}"
                )
                info("-" * 92)
                for model_name in successful_models:
                    r = results[model_name][method]
                    info(
                        f"{model_name:<20} {r['baseline_mse']:<10.4f} {r['baseline_rmse']:<11.4f} "
...
def experiment_one_test_models(
    models_to_test=None, verbose=True, global_test_indices=None
):
    """
    ...
    for idx, (model_name, weight_file_list) in enumerate(model_items, 1):
        ...
        try:
            ...
            for weights_path in weight_file_list:
                ...
                pruning_stats = None
                if "exp1b_" in variant_tag and (
                    "greedy_pruned" in variant_tag or "negative_pruned" in variant_tag
                ):
                    ...
                    if variant_match:
                        ...
                        baseline_weights_path = (
                            WEIGHTS_DIR
                            / f"{model_name}_exp1a_variant{variant_index}_best.pth"
                        )
                        if baseline_weights_path.exists():
                            try:
                                ...
                                pruned_model_for_stats = config["class"](
                                    freeze_backbone=False
                                )
                                pruned_model_for_stats.load_state_dict(
                                    torch.load(
                                        weights_path,
                                        map_location="cpu",
                                        weights_only=True,
                                ...
                                reduction_pct = pruning_stats.get(
                                    "reduction_percentage"
                                )
                                if reduction_pct is not None:
                                    info(
                                        f" - Pruning stats computed for {variant_tag}: reduction {reduction_pct:.2f}%"
                                    )
                            except Exception as stat_err:
                                warn(
                                    f"Failed to compute pruning statistics for {weights_path.name}: {stat_err}"
                                )
                            ...
                            warn(
                                f"Original weights not found for {weights_path.name}: expected {baseline_weights_path.name}"
                            )
                            pruning_stats = {
                                "error": f"Original weights missing: {str(baseline_weights_path)}"
                            }
                    else:
                        warn(
                            f"Could not infer baseline variant for pruned weights {weights_path.name}"
                        )
                        pruning_stats = {
                            "error": "Failed to parse variant index from pruned weight filename."
                        }
                ...
                plcc_str = (
                    f"{test_results['plcc']:.4f}"
                    if test_results["plcc"] is not None
                    else "N/A"
                )
                srcc_str = (
                    f"{test_results['srcc']:.4f}"
                    if test_results["srcc"] is not None
                    else "N/A"
                )
                krcc_str = (
                    f"{test_results['krcc']:.4f}"
                    if test_results["krcc"] is not None
                    else "N/A"
                )
                info(
                    f" - Test MSE: {test_results['mse']:.4f}, RMSE: {test_results['rmse']:.4f}, PLCC: {plcc_str}, SRCC: {srcc_str}, KRCC: {krcc_str}"
                )
    ...
    # Use the number of discovered models as the denominator to avoid errors
    info(
        f"Successfully tested: {len(successful_models)}/{len(weights_files_by_model)} models"
    )

    if successful_models:
        info("Test Results:")
        info(
            f"{'Model':<20} {'Variant':<18} {'Pruning':<10} {'Test MSE':<10} {'Test RMSE':<11} {'PLCC':<8} {'SRCC':<8} {'KRCC':<8}"
        )
        print("-" * 95)
        ...
        for model_name in successful_models:
            ...
            for variant_tag, variant_data in sorted_variants:
                if isinstance(variant_data, dict) and "test_mse" in variant_data:
                    ...
                    plcc_str = (
                        f"{variant_data['plcc']:<8.4f}"
                        if variant_data["plcc"] is not None
                        else f"{'N/A':<8}"
                    )
                    srcc_str = (
                        f"{variant_data['srcc']:<8.4f}"
                        if variant_data["srcc"] is not None
                        else f"{'N/A':<8}"
                    )
                    krcc_str = (
                        f"{variant_data['krcc']:<8.4f}"
                        if variant_data["krcc"] is not None
                        else f"{'N/A':<8}"
                    )

                    info(
                        f"{model_name:<20} {display_variant:<18} {pruning_type:<10} {variant_data['test_mse']:<10.4f} {variant_data['test_rmse']:<11.4f} {plcc_str} {srcc_str} {krcc_str}"
                    )
...
def run_experiment_one_complete(
    ...
    results = {"training": None, "pruning": None, "testing": None}
